...
"updateStyle")},updateContent:function(b,d){function e(){a.updateWidth();d!==!1&&(a.options.position.type!=="static"&&a.updatePosition(a.elements.tooltip.is(":visible"),!0),a.options.style.tip.corner!==!1&&v.call(a))}var f,g,h;if(a.status.rendered){if(!b)return c.fn.qtip.log.error.call(a,2,c.fn.qtip.constants.NO_CONTENT_PROVIDED,"updateContent")}else return c.fn.qtip.log.error.call(a,2,c.fn.qtip.constants.TOOLTIP_NOT_RENDERED,"updateContent");f=a.beforeContentUpdate.call(a,b);if(typeof f=="string")b=
f;else if(f===!1)return;c.browser.msie&&a.elements.contentWrapper.children().css({zoom:"normal"});b.jquery&&b.length>0?b.clone(!0).appendTo(a.elements.content).show():a.elements.content.html(b);g=a.elements.content.find("img[complete=false]");g.length>0?(h=0,g.each(function(){c('<img src="'+c(this).attr("src")+'" />').load(function(){++h==g.length&&e()})})):e();a.onContentUpdate.call(a);return c.fn.qtip.log.error.call(a,1,c.fn.qtip.constants.EVENT_CONTENT_UPDATED,"loadContent")},loadContent:function(b,
...
this.elements.tooltip=c(this.elements.tooltip);this.elements.tooltip.appendTo(this.options.position.container);this.elements.tooltip.data("qtip",{current:0,interfaces:[this]});this.elements.wrapper=this.elements.tooltip.children("div:first");this.elements.contentWrapper=this.elements.wrapper.children("div:first").css({background:this.options.style.background});this.elements.content=this.elements.contentWrapper.children("div:first").css(p(this.options.style));c.browser.msie&&this.elements.wrapper.add(this.elements.content).css({zoom:1});
...
a){b=b.get(0).getContext("2d");b.fillStyle=a;b.beginPath();b.arc(c[0],c[1],d,0,Math.PI*2,!1);b.fill()}function x(b){var e,d,a,j;this.elements.tip!==null&&this.elements.tip.remove();e=this.options.style.tip.color||this.options.style.border.color;if(this.options.style.tip.corner!==!1){if(!b)b=this.options.style.tip.corner;d=r(b,this.options.style.tip.size.width,this.options.style.tip.size.height);this.elements.tip='<div class="'+this.options.style.classes.tip+'" dir="ltr" rel="'+b+'" style="position:absolute; height:'+
...
var d,a,j,n;d=this;a=d.options.show.when.target;j=d.options.hide.when.target;d.options.hide.fixed&&(j=j.add(d.elements.tooltip));if(d.options.hide.when.event=="inactive"){n="click,dblclick,mousedown,mouseup,mousemove,mouseout,mouseenter,mouseleave,mouseover".split(",");var m=function(a){if(d.status.disabled!==!0)clearTimeout(d.timers.inactive),d.timers.inactive=setTimeout(function(){c(n).each(function(){j.unbind(this+".qtip-inactive");d.elements.content.unbind(this+".qtip-inactive")});d.hide(a)},
...
d.dimensions.height:this.cache.mouse.y-d.dimensions.height,g="bottom";if(l.left<0)l.left=b.left,f=!1;if(l.top<0)l.top=b.top,g=!1;if(this.options.style.tip.corner!==!1){l.corner=new String(d.corner);if(f!==!1)l.corner=l.corner.replace(/Left|Right|Middle/,f);if(g!==!1)l.corner=l.corner.replace(/top|bottom/,g);l.corner!==this.elements.tip.attr("rel")&&x.call(this,l.corner)}return l}function p(b,e){var d,a;d=c.extend(!0,{},b);for(a in d)e===!0&&a.search(/(tip|classes)/i)!==-1?delete d[a]:!e&&a.search(/(width|border|tip|title|classes|user)/i)!==
-1&&delete d[a];return d}function y(b){if(typeof b.tip!=="object")b.tip={corner:b.tip};if(typeof b.tip.size!=="object")b.tip.size={width:b.tip.size,height:b.tip.size};if(typeof b.border!=="object")b.border={width:b.border};if(typeof b.width!=="object")b.width={value:b.width};if(typeof b.width.max=="string")b.width.max=parseInt(b.width.max.replace(/([0-9]+)/i,"$1"));if(typeof b.width.min=="string")b.width.min=parseInt(b.width.min.replace(/([0-9]+)/i,"$1"));if(typeof b.tip.size.x=="number")b.tip.size.width=
...
"object")b.hide.when={event:b.hide.when};if(typeof b.hide.effect!=="object")b.hide.effect={type:b.hide.effect};if(typeof b.style!=="object")b.style={name:b.style};b.style=y(b.style);n=c.extend(!0,{},c.fn.qtip.defaults,b);n.style=q.call({options:n},n.style);n.user=c.extend(!0,{},b)}return c(this).each(function(){if(typeof b=="string"){if(f=b.toLowerCase(),j=c(this).qtip("interfaces"),typeof j=="object")if(e===!0&&f=="destroy")for(;j.length>0;)j[j.length-1].destroy();else{e!==!0&&(j=[c(this).qtip("api")]);
for(d=0;d<j.length;d++)f=="destroy"?j[d].destroy():j[d].status.rendered===!0&&(f=="show"?j[d].show():f=="hide"?j[d].hide():f=="focus"?j[d].focus():f=="disable"?j[d].disable(!0):f=="enable"&&j[d].disable(!1))}}else{g=c.extend(!0,{},n);g.hide.effect.length=n.hide.effect.length;g.show.effect.length=n.show.effect.length;if(g.position.container===!1)g.position.container=c(document.body);if(g.position.target===!1)g.position.target=c(this);if(g.show.when.target===!1)g.show.when.target=c(this);if(g.hide.when.target===
!1)g.hide.when.target=c(this);a=c.fn.qtip.interfaces.length;for(d=0;d<a;d++)if(typeof c.fn.qtip.interfaces[d]=="undefined"){a=d;break}m=new z(c(this),g,a);c.fn.qtip.interfaces[a]=m;if(typeof c(this).data("qtip")=="object"){if(typeof c(this).attr("qtip")==="undefined")c(this).data("qtip").current=c(this).data("qtip").interfaces.length;c(this).data("qtip").interfaces.push(m)}else c(this).data("qtip",{current:0,interfaces:[m]});g.content.prerender===!1&&g.show.when.event!==!1&&g.show.ready!==!0?g.show.when.target.bind(g.show.when.event+
".qtip-"+a+"-create",{qtip:a},function(a){h=c.fn.qtip.interfaces[a.data.qtip];h.options.show.when.target.unbind(h.options.show.when.event+".qtip-"+a.data.qtip+"-create");h.cache.mouse={x:a.pageX,y:a.pageY};w.call(h);h.options.show.when.target.trigger(h.options.show.when.event)}):(m.cache.mouse={x:g.show.when.target.offset().left,y:g.show.when.target.offset().top},w.call(m))}})};c(document).ready(function(){c.fn.qtip.cache={screen:{scroll:{left:c(window).scrollLeft(),top:c(window).scrollTop()},width:c(window).width(),
height:c(window).height()}};var b;c(window).bind("resize scroll",function(e){clearTimeout(b);b=setTimeout(function(){e.type==="scroll"?c.fn.qtip.cache.screen.scroll={left:c(window).scrollLeft(),top:c(window).scrollTop()}:(c.fn.qtip.cache.screen.width=c(window).width(),c.fn.qtip.cache.screen.height=c(window).height());for(i=0;i<c.fn.qtip.interfaces.length;i++){var b=c.fn.qtip.interfaces[i];b.status.rendered===!0&&(b.options.position.type!=="static"||b.options.position.adjust.scroll&&e.type==="scroll"||
b.options.position.adjust.resize&&e.type==="resize")&&b.updatePosition(e,!0)}},100)});c(document).bind("mousedown.qtip",function(b){c(b.target).parents("div.qtip").length===0&&c(".qtip[unfocus]").each(function(){var d=c(this).qtip("api");c(this).is(":visible")&&!d.status.disabled&&c(b.target).add(d.elements.target).length>1&&d.hide(b)})})});c.fn.qtip.interfaces=[];c.fn.qtip.log={error:function(){return this}};c.fn.qtip.constants={};c.fn.qtip.defaults={content:{prerender:!1,text:!1,url:!1,data:null,
